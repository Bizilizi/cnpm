...
def sample_uniform_points_in_sphere(amount, sphere_size=1):
    unit_sphere_points = np.random.uniform(-1, 1, size=(amount * 2 + 20, 3))
    unit_sphere_points = unit_sphere_points[
        np.linalg.norm(unit_sphere_points, axis=1) < sphere_size
    ]
    ...
    if points_available < amount:
        ...
        result[points_available:, :] = sample_uniform_points_in_sphere(
...
def sample_uniform_points_in_box(amount, box_size=1):
    box_points = np.random.uniform(-box_size, box_size, size=(amount, 3))
    return box_points
