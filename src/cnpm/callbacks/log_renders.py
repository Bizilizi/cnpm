...
class LogRendersCallback(Callback):
    ...
    def on_train_epoch_end(
        ...
        if not (model.with_vr_color_loss or model.with_vr_silhouette_loss):
            return
        ...
        if self.state["epochs"] % self.log_every == 0:
            ...
            for i in range(num_boards):
                ...
                color_latent_code = color_latent_vectors[i * model.color_latent_num, :]
                ...
                with torch.no_grad():
                    for elev, azim in zip(target_elev, target_azim):
                        (
                        ...
                        predicted_colors_at_rays = (predicted_colors_at_rays + 1) / 2
